...
const nyro = require("nyro").default;
...
try{
var api = await nyro.get(`https://hercai.onrender.com/${model}/hercai?question=`+encodeURI(content),{
    headers: {
...
async drawImage({model = "v3",prompt,negative_prompt=""}){
    ...
    try{
    var api = await nyro.get(`https://hercai.onrender.com/${model}/text2image`+"?prompt="+encodeURI(prompt)+"&negative_prompt="+encodeURI(negative_prompt),{
        headers: {
            "content-type": "application/json",
            "Authorization": this.apiKey,
        },
...
async betaQuestion({content,personality="",user=""}){
    if(!content || content == undefined || content == null)throw new Error("Please specify a question!");
    try{
    var api = await nyro.get(`https://hercai.onrender.com/beta/hercai?question=`+encodeURI(content)+`&user=`+encodeURI(user),{
    headers: {
        "content-type": "application/json",
        "Authorization": this.apiKey,
    },
...
if(!prompt || prompt == undefined || prompt == null)throw new Error("Please specify a prompt!");
try{
    var api = await nyro.get(`https://hercai.onrender.com/beta/text2image`+"?prompt="+encodeURI(prompt)+"&negative_prompt="+encodeURI(negative_prompt)+"&sampler="+encodeURI(sampler)+"&image_style="+encodeURI(image_style)+"&width="+width+"&height="+height+"&steps="+steps+"&scale="+scale,{
        headers: {
            "content-type": "application/json",
            "Authorization": this.apiKey,
        },
